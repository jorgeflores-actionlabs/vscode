...
SP_CALL = "EXEC [dbo].[SQS_ProcessDealerStagedTable] ?"
    ...
    try:
        ...
        for index, dealer_id in enumerate(dealer_ids, start=1):
            print(
                ...
                f"SQS_ProcessDealerStagedTable for DealerId={dealer_id}..."
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Execute SQS_ProcessDealerStagedTable. Without a DealerId, values are "
            "read from input.txt."
        )
    )
    parser.add_argument(
...
if __name__ == "__main__":
    # File mode:          python 2_xsl.py
    # Single-dealer mode: python 2_xsl.py 1785
    main()
